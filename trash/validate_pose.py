...
def get_last_epoch_and_weights_file():
    return -1, 'weights-canonical/weights.best.h5'
...
if use_client_gen:
    ...
    val_client = DataGeneratorClient(port=5556, host="localhost", hwm=160, batch_size=batch_size, limit = val_samples)
else:
    train_client = DataIterator("../dataset/train_dataset.h5", shuffle=True, augment=True, batch_size=batch_size)
    val_client = DataIterator("../dataset/val_dataset.h5", shuffle=False, augment=False, batch_size=batch_size, limit = val_samples)
